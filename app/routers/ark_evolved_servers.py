"""
Ark Survival Evolved Server router - Server Admin Ark Survival Evolved szerver kezelés
"""
...
router = APIRouter(prefix="/ark-evolved", tags=["ark_evolved_servers"])
            ...
            with shutdown_lock:
                if server_id in scheduled_shutdowns:
                    ...
                    if datetime.now() >= shutdown_time:
                        ...
                        container_name = f"zedin_ase_{server_id}"
                        
                        try:
                            # Ellenőrizzük, hogy a folyamat még fut-e
                            if check_process_running_in_container(container_name, "ShooterGameServer"):
                            ...
                            process_stopped = wait_for_process_shutdown(container_name, "ShooterGameServer", max_wait_seconds=30)
...
async def list_clusters(
    ...
    return templates.TemplateResponse("ark_evolved/clusters.html", {
        "request": request,
...
async def show_create_cluster(
    ...
    return templates.TemplateResponse("ark_evolved/cluster_create.html", {
        "request": request,
        "current_user": current_user
    })
...
async def create_cluster(
    ...
    return RedirectResponse(
        url="/ark-evolved/clusters?success=Cluster+létrehozva",
        status_code=302
    )
...
async def delete_cluster(
    ...
    if servers_count > 0:
        return RedirectResponse(
            url=f"/ark-evolved/clusters?error=A+cluster+törlése+előtt+először+törölni+kell+a+hozzárendelt+szerevereket+({servers_count}+szerver)",
            status_code=302
        )
    ...
    return RedirectResponse(
        url="/ark-evolved/clusters?success=Cluster+törölve",
        status_code=302
    )
...
async def show_create_server(
    ...
    if not ark_game:
        raise HTTPException(
            status_code=404,
            detail="Ark Survival Evolved játék nem található. Kérlek, vedd fel a kapcsolatot a Manager Adminisztrátorral."
        )
    ...
    if not clusters:
        return RedirectResponse(
            url="/ark-evolved/clusters/create?error=Először+hozz+létre+egy+Cluster-t!",
            status_code=302
        )
    ...
    return templates.TemplateResponse("ark_evolved/server_create.html", {
        "request": request,
        "current_user": current_user,
        "ark_game": ark_game,
        "clusters": clusters,
...
async def create_server(
    ...
    return RedirectResponse(
        url="/ark-evolved/servers?success=Szerver+létrehozva",
        status_code=302
    )
...
async def list_servers(
    ...
    return templates.TemplateResponse("ark_evolved/servers.html", {
        "request": request,
        "current_user": current_user,
...
async def show_edit_server(
    ...
    return templates.TemplateResponse("ark_evolved/server_edit.html", {
        "request": request,
        "current_user": current_user,
        "server": server,
        "clusters": clusters
...
async def delete_server(
    ...
    if server.status == ServerStatus.RUNNING:
        ...
        try:
            ...
            if not stop_result.get("success"):
                return RedirectResponse(
                    url=f"/ark-evolved/servers?error=A+szerver+leállítása+sikertelen.+Kérem,+állítsa+le+manuálisan",
                    status_code=302
                )
            ...
            return RedirectResponse(
                url=f"/ark-evolved/servers?error=A+szerver+törlése+előtt+először+le+kell+állítani",
                status_code=302
            )
    ...
    return RedirectResponse(
        url="/ark-evolved/servers?success=Szerver+törölve",
        status_code=302
    )
...
async def edit_server(
    ...
    return RedirectResponse(
        url=f"/ark-evolved/servers?success=Szerver+módosítva",
        status_code=302
    )
...
async def start_server_endpoint(
    ...
    if not server.token_used_id:
        return RedirectResponse(
            url=f"/ark-evolved/servers?error=Nincs token hozzárendelve a szerverhez. A szerver nem indítható.",
            status_code=302
        )
    ...
    if not token:
        return RedirectResponse(
            url=f"/ark-evolved/servers?error=A token nem található. A szerver nem indítható.",
            status_code=302
        )
    
    if not token.is_active or (token.expires_at and token.expires_at <= datetime.utcnow()):
        return RedirectResponse(
            url=f"/ark-evolved/servers?error=A token lejárt vagy inaktív. A szerver nem indítható.",
            status_code=302
        )
    ...
    if server.token_expires_at and server.token_expires_at <= datetime.utcnow():
        return RedirectResponse(
            url=f"/ark-evolved/servers?error=A token lejárt. A szerver nem indítható.",
            status_code=302
        )
    ...
    if result["success"]:
        return RedirectResponse(
            url=f"/ark-evolved/servers?success={result['message']}",
            status_code=302
        )
    else:
        return RedirectResponse(
            url=f"/ark-evolved/servers?error={result['message']}",
            status_code=302
        )
...
async def set_ram_limit(
    ...
    return RedirectResponse(
        url=f"/ark-evolved/servers?success=RAM+limit+beállítva",
        status_code=302
    )
